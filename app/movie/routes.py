...
@mov.route('/rank/<string:name>', methods = ['POST', "GET"])
@login_required
def rank(name):
    if request.method == "POST":
        print("POST")
        ...
        found = True if any(name == rank.movie_name for rank in ranks) else False
        if found:
            flash("Movie is already ranked!", category='error')
        else:
            if 0 < int(rank_no) <= 20:
                rank = RankMovie(rank = rank_no, movie_name = name, userid = current_user.id)
                db.session.add(rank)
                db.session.commit()
                return redirect(url_for('mov.listofmovies'))
            else:
